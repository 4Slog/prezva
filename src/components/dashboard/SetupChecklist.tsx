  ...
  return (
    <div className="rounded-xl border border-[#1E3A5F] bg-[#112240] p-6">
      ...
      <div className="h-1.5 bg-[#1E3A5F] rounded-full mb-5 overflow-hidden">
        <div
          className="h-full bg-[#00BFA6] rounded-full transition-all"
      ...
      <ul className="space-y-3">
        {items.map((item, i) => (
          <li key={i} className="flex items-center gap-3">
            <span className={`h-5 w-5 flex-shrink-0 rounded-full flex items-center justify-center text-xs font-bold ${item.done ? 'bg-[#00BFA6] text-[#0D1B2A]' : 'border border-[#1E3A5F] text-[#64748B]'}`}>
              {item.done ? '✓' : i + 1}
            </span>
            {item.href && !item.done ? (
              <a href={item.href} className="text-sm text-[#00BFA6] hover:underline">{item.label}</a>
            ) : (
