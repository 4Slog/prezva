      ...
      <ul className="space-y-3">
        {items.map((item, i) => (
          <li key={i} className="flex items-center gap-3">
            <span className={`h-5 w-5 flex-shrink-0 rounded-full flex items-center justify-center text-xs font-bold ${item.done ? 'bg-[#2DD4BF] text-[var(--pz-on-accent)]' : 'border border-[var(--pz-border)] text-[var(--pz-muted)]'}`}>
              {item.done ? '✓' : i + 1}
            </span>
            {item.href && !item.done ? (
