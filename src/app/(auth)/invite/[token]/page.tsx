...
export default async function InvitePage({ params }: Props) {
  ...
  if (!user) {
    ...
    return (
      <div className="pz-card p-8 text-center">
        ...
        <p className="text-xs mb-6" style={{ color: 'var(--pz-muted)' }}>
          Sign in (or create an account) with <strong>{invite.email}</strong> to accept.
        </p>
        <div className="flex flex-col gap-3">
          <Link
            href={`/login?next=${next}`}
            className="w-full py-2 rounded-lg font-semibold text-sm text-center block"
            style={{
  ...
  return (
    <div className="pz-card p-8 text-center">
      ...
      <p className="text-xs mt-4" style={{ color: 'var(--pz-muted)' }}>
        Signed in as <strong>{user.email}</strong>
        {user.email?.toLowerCase() !== invite.email.toLowerCase() && (
          <span style={{ color: 'var(--pz-error)' }}> — this invite was sent to {invite.email}</span>
        )}
      </p>
    </div>
  )
