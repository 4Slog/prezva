...
import { orgCheckInToSession, orgOverrideSessionCheckIn } from '@/lib/checkin/actions'
...
export default function SessionCheckInClient({
  ...
  const handleQRScan = useCallback(async (code: string) => {
    ...
    const result = await orgCheckInToSession(eventId, sessionId, code, 'qr_scan')
    showResult(result)
  ...
  async function handleManualToggle(registrationId: string, currentlyCheckedIn: boolean) {
    ...
    if (overrideMode) {
      const result = await orgOverrideSessionCheckIn(eventId, sessionId, registrationId)
