  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        {[
          { label: 'Attendees',     href: `/events/${slug}/attendees`,     icon: '👥' },
          { label: 'Agenda',        href: `/events/${slug}/agenda`,         icon: '📋' },
          { label: 'Check-in',      href: `/events/${slug}/checkin`,        icon: '✅' },
          { label: 'Announcements', href: `/events/${slug}/announcements`,  icon: '📣' },
          { label: 'Surveys',       href: `/events/${slug}/surveys`,        icon: '📊' },
          { label: 'Analytics',     href: `/events/${slug}/analytics`,      icon: '📈' },
        ].map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="pz-card p-4 flex items-center gap-3 hover:border-[#00BFA6]/40 transition-colors"
          >
