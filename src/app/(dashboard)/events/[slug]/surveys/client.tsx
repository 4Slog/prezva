...
export default function SurveysClient({ surveys: init, eventId, slug }: {
  ...
  return (
    <div>
      {!showCreate && (
        <button onClick={() => setShowCreate(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--pz-teal)', color: '#fff', border: 'none', borderRadius: 8, padding: '0.6rem 1.25rem', fontWeight: 600, cursor: 'pointer', marginBottom: '1.5rem' }}>
          <Plus size={16} /> New Survey
        </button>
      )}
      {showCreate && (
        <form onSubmit={handleCreate} style={{ border: '1px solid var(--pz-border)', borderRadius: 12, padding: '1.5rem', marginBottom: '1.5rem', background: 'var(--pz-surface)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
            <input name="title" required placeholder="Survey title..." style={{ padding: '0.6rem 0.75rem', borderRadius: 8, border: '1px solid var(--pz-border)', background: 'var(--pz-bg)', color: 'var(--pz-text)', fontSize: 14 }} />
            <textarea name="description" rows={2} placeholder="Optional description..." style={{ padding: '0.6rem 0.75rem', borderRadius: 8, border: '1px solid var(--pz-border)', background: 'var(--pz-bg)', color: 'var(--pz-text)', fontSize: 14, resize: 'vertical' }} />
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="submit" disabled={isPending} style={{ background: 'var(--pz-teal)', color: '#fff', border: 'none', borderRadius: 8, padding: '0.6rem 1.25rem', fontWeight: 600, cursor: 'pointer' }}>Create</button>
              <button type="button" onClick={() => setShowCreate(false)} style={{ background: 'var(--pz-border)', color: 'var(--pz-text)', border: 'none', borderRadius: 8, padding: '0.6rem 1.25rem', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        </form>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {surveys.length === 0 && <p style={{ color: 'var(--pz-muted)', textAlign: 'center', padding: '3rem 0' }}>No surveys created yet.</p>}
        {surveys.map(s => {
          const color = STATUS_COLOR[s.status] ?? '#6b7280'
          const isOpen = expanded === s.id
          return (
            <div key={s.id} style={{ border: '1px solid var(--pz-border)', borderRadius: 10, background: 'var(--pz-surface)' }}>
              <div style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  {s.description && <p style={{ fontSize: 13, color: 'var(--pz-muted)', marginTop: 2 }}>{s.description}</p>}
                </div>
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  ...
                  <button onClick={() => setExpanded(isOpen ? null : s.id)} style={{ background: 'var(--pz-border)', color: 'var(--pz-text)', border: 'none', borderRadius: 6, padding: '4px 8px', cursor: 'pointer' }}>
