...
interface LeaderEntry { userId: string; name: string; count: number }
        ...
        <div className="pz-card p-6">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {leaderboard.map((entry, i) => (
              <div key={entry.userId} style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 14 }}>
                <span style={{ fontWeight: 700, color: 'var(--pz-muted)', width: 20, textAlign: 'right' }}>{i + 1}</span>
                <span style={{ flex: 1 }}>{entry.name}</span>
                <span style={{ fontWeight: 600, color: 'var(--pz-teal)' }}>{entry.count} stamp{entry.count !== 1 ? 's' : ''}</span>
              </div>
            ))}
          </div>
        </div>
