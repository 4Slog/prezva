...
export function DayOfInfoSection({ eventId, initialValue }: Props) {
  ...
  return (
    <div className="pz-card p-4 mt-6">
      <Field
        label="Day-of info for speakers"
        htmlFor="day-of-info"
        helper="Shown to speakers in their hub on the day of the event."
      >
        <textarea
          id="day-of-info"
          value={value}
      ...
      {saved && <span className="text-xs mt-1 block" style={{ color: 'var(--pz-success)' }}>Saved</span>}
    </div>
  )
}
