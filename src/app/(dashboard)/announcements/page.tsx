...
export const metadata = { title: 'Announcements — Prezva' }
...
export default async function AnnouncementsPage() {
  ...
  if (!events || events.length === 0) {
    return (
      <div>
        <h1 className="text-2xl font-bold mb-6" style={{ color: 'var(--pz-text)' }}>Announcements</h1>
        <EmptyState
          icon="📣"
          title="No events yet"
          description="Create an event to send announcements to your attendees."
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-2" style={{ color: 'var(--pz-text)' }}>Announcements</h1>
      <p className="text-sm mb-6" style={{ color: 'var(--pz-muted)' }}>
        Select an event to send announcements to its attendees.
      </p>
      ...
      <div className="grid gap-3">
        {events.map((event) => (
          <a
            key={event.id}
            href={`/events/${event.slug}/announcements`}
