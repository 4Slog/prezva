        ...
        <div key={day} style={{ marginBottom:'2.5rem' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {daySessions.map(s => {
              ...
              return (
                <div key={s.id} style={{ border:'1px solid var(--pz-border)', borderRadius:10, padding:'1rem 1.25rem', background:'var(--pz-surface)', borderLeft:borderStyle, display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div style={{ flex:1 }}>
                    ...
                    {spks.length > 0 && <p style={{ fontSize:13, color:'var(--pz-muted)' }}>{(spks as any[]).map(sp => sp.name).join(', ')}</p>}
                  </div>
                  <button onClick={() => handleBookmark(s.id)} style={{ background:'none', border:'none', cursor:'pointer', color: bookmarks.has(s.id) ? 'var(--pz-teal)' : 'var(--pz-muted)', padding:'4px', flexShrink:0 }}>
