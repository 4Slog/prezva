              ...
              {speakers.map((sp: any) => (
                <Link key={sp.id} href={'/e/'+slug+'/speakers/'+sp.id} style={{ textDecoration:'none', color:'inherit' }}>
                  <div style={{ border:'1px solid var(--pz-border)', borderRadius:14, padding:'1.5rem 1.25rem', textAlign:'center', background:'var(--pz-surface)' }}>
                    <div style={{ width:72, height:72, borderRadius:'50%', background:'var(--pz-surface)', margin:'0 auto 1rem', display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, color:'var(--pz-teal)', fontWeight:700, overflow:'hidden' }}>
                    ...
                    {sp.company && <p style={{ fontSize:13, color:'var(--pz-muted)' }}>{sp.company}</p>}
                  </div>
                </Link>
              ))}
            </div>}
