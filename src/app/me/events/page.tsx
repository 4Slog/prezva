...
function StatusBadge({ status }: { status: string | null }) {
  const color = status === 'live' ? 'var(--pz-success-fill)' : status === 'published' ? '#2DD4BF' : status === 'ended' ? '#64748B' : '#94A3B8'
  return (
...
function RolePillLink({ pill }: { pill: RolePill }) {
  if (pill.type === 'attendee') {
    return (
      <Link href={`/e/${pill.eventSlug}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: '#22c55e22', color: 'var(--pz-success-fill)', textDecoration: 'none', border: '1px solid #22c55e44' }}>
  ...
  if (pill.type === 'speaker') {
    return (
      <Link href={`/speaker/${pill.token}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: 'var(--pz-teal, #2DD4BF)22', color: 'var(--pz-teal, #2DD4BF)', textDecoration: 'none', border: '1px solid var(--pz-teal, #2DD4BF)44' }}>
        🎙️ Speaker{pill.speakerRole ? ` · ${pill.speakerRole}` : ''}
      </Link>
    )
  }
  return (
    <Link href={`/volunteer/${pill.token}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: '#f59e0b22', color: 'var(--pz-warning-fill)', textDecoration: 'none', border: '1px solid #f59e0b44' }}>
