...
class SharePointAdapter implements IntegrationAdapter {
  ...
  async handleCallback(code: string, orgId: string, redirectUri: string): Promise<void> {
    ...
    try {
      ...
      const { error: saveError } = await supabase.from('org_integrations').upsert({ org_id: orgId, provider: PROVIDER, status: 'connected', encrypted_refresh_token: encryptedToken, scopes: tokens.scope?.split(' ') ?? SCOPES, last_synced_at: new Date().toISOString() }, { onConflict: 'org_id,provider' })
  ...
  async getAccessToken(orgId: string): Promise<string | null> {
    ...
    try {
      ...
      return refreshAccessToken(PROVIDER, decrypted, (process.env.SHAREPOINT_CLIENT_ID ?? process.env.MICROSOFT_CLIENT_ID)!, (process.env.SHAREPOINT_CLIENT_SECRET ?? process.env.MICROSOFT_CLIENT_SECRET)!)
