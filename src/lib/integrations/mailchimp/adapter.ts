...
class MailchimpAdapter implements IntegrationAdapter {
  ...
  async handleCallback(code: string, orgId: string, redirectUri: string): Promise<void> {
    ...
    try {
      ...
      const { error: saveError } = await supabase.from('org_integrations').upsert({ org_id: orgId, provider: PROVIDER, status: 'connected', encrypted_refresh_token: encryptedToken, directionality_preferences: { dc }, last_synced_at: new Date().toISOString() }, { onConflict: 'org_id,provider' })
