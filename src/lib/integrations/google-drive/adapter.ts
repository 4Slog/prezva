...
class GoogleDriveAdapter implements IntegrationAdapter {
  ...
  async getAccessToken(orgId: string): Promise<string | null> {
    ...
    try {
      return refreshAccessToken(PROVIDER, decryptToken(data.encrypted_refresh_token), (process.env.GOOGLE_DRIVE_CLIENT_ID ?? process.env.GOOGLE_CLIENT_ID)!, (process.env.GOOGLE_DRIVE_CLIENT_SECRET ?? process.env.GOOGLE_CLIENT_SECRET)!)
